  ...
  const handleProfileUpdate = async (updatedData) => {
    ...
    try {
      ...
      if (role === 'ngo') {
        ...
        } else if (updatedData.avatar && typeof updatedData.avatar === 'string' && updatedData.avatar.startsWith('http')) {
          // It's a URL (e.g. dicebear or google), pas it.
          // We need to confirm if backend accepts an 'avatarUrl' or if we can overload 'avatar' field.
          // Looking at VolunteerProfile model, `avatar` is a String.
          // Looking at backend `profile.js` route (implied), it likely checks `req.file` for 'avatar'.
          // We might need to send `avatar` as text field if it's a URL.
          // Let's send it as `avatarUrl` and also `avatar` string if Multer doesn't block it.
          formData.append('avatarUrl', updatedData.avatar);
        }
    ...
    <div className="profile-shell">
      <div className="profile-card">
        <div className="profile-header">
          ...
          <div>
            <h1 className="text-3xl font-normal text-[#123524] tracking-tight">Profile Settings</h1>
            <p className="text-gray-500 font-normal mt-1">{role.charAt(0).toUpperCase() + role.slice(1)} Account</p>
          </div>
        </div>
