        ...
        <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end pointer-events-none">
                ...
                <div className="bg-white rounded-2xl shadow-2xl border border-gray-200 w-[350px] md:w-[380px] h-[500px] flex flex-col mb-4 pointer-events-auto overflow-hidden animate-in slide-in-from-bottom-10 fade-in duration-300">
                    ...
                    <div className="bg-[#123524] p-4 flex items-center justify-between text-white rounded-t-2xl">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h3 className="font-bold text-base leading-tight">EcoBot Assistant</h3>
                        ...
                        {messages.map((msg) => (
                            <div key={msg.id} className={`flex ${msg.type === 'user' ? 'justify-end' : 'justify-start'}`}>
                                <div className={`max-w-[80%] rounded-2xl p-3 text-sm shadow-sm ${msg.type === 'user'
                                    ? 'bg-green-600 text-white !text-white rounded-br-none'
                                    : 'bg-white text-gray-800 border border-gray-100 rounded-bl-none'
                                    }`}>
                                    <p className="whitespace-pre-wrap leading-relaxed">{msg.text}</p>
                                    <p className={`text-[10px] mt-1 text-right ${msg.type === 'user' ? 'text-green-100' : 'text-gray-400'}`}>{msg.time}</p>
                                </div>
                            </div>
                        ))}
                        {loading && (
                    ...
                    {messages.length === 1 && (
                        <div className="px-4 py-2 bg-gray-50 border-t border-gray-100 overflow-x-auto whitespace-nowrap scrollbar-none flex gap-2">
                            <button onClick={() => handleQuickReply("Find volunteer opportunities")} className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-green-200 text-green-700 rounded-full text-xs font-medium hover:bg-green-50 transition-colors shadow-sm">
                                <Zap size={12} /> Opportunities
                            </button>
                            <button onClick={() => handleQuickReply("My impact report")} className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-blue-200 text-blue-700 rounded-full text-xs font-medium hover:bg-blue-50 transition-colors shadow-sm">
            ...
            {!isOpen && (
                ...
                >
                    ...
                    {/* Notification Badge */}
                    <span className="absolute top-0 right-0 w-5 h-5 bg-red-500 border-2 border-white rounded-full flex items-center justify-center">
                        <span className="w-2 h-2 bg-white rounded-full animate-ping absolute opacity-75"></span>
