  ...
  try {
    ...
    try {
      await AdminLog.create({
        ...
        targetUser: user._id,
        details: `NGO ${user.username} created opportunity: ${title}`
      });
    } catch (logErr) {
  ...
  try {
    ...
    try {
      await AdminLog.create({
        ...
        performedBy: user._id,
        targetUser: user._id,
        details: `NGO ${user.username} updated opportunity: ${op.title}`
      });
    } catch (logErr) {
      console.error('Failed to create audit log:', logErr);
    }
  ...
  try {
    ...
    try {
      await AdminLog.create({
        ...
        performedBy: user._id,
        targetUser: user._id,
        details: `NGO ${user.username} deleted opportunity: ${opTitle}`
      });
    } catch (logErr) {
      console.error('Failed to create audit log:', logErr);
    }
...
// GET /api/opportunities/completed-pending-feedback
router.get('/completed-pending-feedback', protect, async (req, res) => {
  try {
    const todayStr = new Date().toISOString().split('T')[0];

    let query = { date: { $lt: todayStr } };

    if (req.user.role === 'ngo') {
      query.ngo_id = req.user.id;
    } else if (req.user.role === 'volunteer') {
      const myApps = await Application.find({ volunteer_id: req.user.id, status: 'accepted' });
      const myOppIds = myApps.map(a => a.opportunity_id);
      query._id = { $in: myOppIds };
    } else {
      return res.json({ success: true, data: [] });
    }

    const completedOpps = await Opportunity.find(query);
