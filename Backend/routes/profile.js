...
router.put('/', protect, upload.fields([{ name: 'logo', maxCount: 1 }, { name: 'avatar', maxCount: 1 }]), async (req, res, next) => {
  try {
    ...
    const { fullName, location, skills, username } = req.body;
    if (fullName) {
      // Ensure fullName is a string (handles cases where frontend sends it as an array)
      user.fullName = Array.isArray(fullName) ? fullName[0] : String(fullName);
    }
    if (username) user.username = username; // Allow updating username
    ...
    // 2. Logic Split based on Role + Handle Avatar Sync for ALL roles
    let specificProfile = null;

    // Capture avatar for User model sync
    let unifiedAvatar = req.body.avatarUrl || user.avatar;
    if (req.files) {
      if (req.files.logo && req.files.logo[0]) unifiedAvatar = `/uploads/logos/${req.files.logo[0].filename}`;
      if (req.files.avatar && req.files.avatar[0]) unifiedAvatar = `/uploads/avatars/${req.files.avatar[0].filename}`;
    }

    // Sync to master User model
    if (unifiedAvatar) {
      user.avatar = unifiedAvatar;
      await user.save();
    }
    ...
    if (user.role === 'ngo') {
      ...
      const ngoFields = {
        ...
        city,
        country,
        logo: unifiedAvatar // Sync logo with unified avatar
      };
      ...
      const volFields = {
        ...
        avatar: unifiedAvatar // Sync avatar with unified avatar
      };
